...
- **PR + auto-merge** когда фича готова. Verification-skill, code-review skill, reflexion — **опциональны**, не обязательны. Зови их если хочешь.
...
Меняется кем угодно — но **скажи команде в чате/PR-комменте**, что меняешь. Без формальной RFC-PR-церемонии, без обязательных аппрувов. Если кто-то не согласен — обсуждаем.

Принцип: одна осмысленная единица изменения = один PR. Не суй правки общей зоны «попутно» внутрь фичи — отдельный коммит/PR.

### Дизайн-конвенция (фронт)
- Только токены, не хардкоды. `bg-primary` ✅, `bg-[#0EA5E9]` ❌.
- Только компоненты из `components/ui/`. Если нужного нет — добавь и скажи команде.
