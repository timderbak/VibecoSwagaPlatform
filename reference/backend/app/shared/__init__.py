"""
Общая зона (CODEOWNERS = все).

Сюда идут публичные контракты (Pydantic Read-схемы), общие enum'ы и базовые
типы ответов API. Любое изменение здесь — только через RFC-PR (см. CLAUDE.md §17).
...
Зачем такая структура:
  - Профиль владеет UserRead, но он лежит в shared/ — потому что Finances и AI читают User.
  - Owner модуля решает форму схемы; через RFC-PR с аппрувом всех CODEOWNERS общей зоны.
