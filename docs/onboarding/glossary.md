...
- **Module** — крупный продуктовый домен (`Projects`, `Finances`, `Profile`, `AI`). Один Dev владеет 1+ модулями.
- **Submodule** — логический кусок модуля (например, `Project core`, `Milestones`, `Deliverables` внутри `Projects`). Развиваются последовательно.
- **Feature** — одна конкретная фича внутри подмодуля (один PR, один integration-test, ~день работы).
- **Sub-task** — атомарная TDD-итерация внутри Feature (один коммит).

## Slice (устаревший термин)

Раньше использовалось «slice» для вертикального среза. Теперь — **Module** (с подмодулями и фичами). Слайс ≈ Module + его подпапки в backend/frontend.
