...
Никакого cross-approve, никакого CODEOWNERS, никакой обязательной verification-фазы перед мержем. Каждый отвечает за свою зону.

В чужой модуль — через GitHub-issue с assignee на owner'а. Изменение общей зоны (`backend/app/shared/`, `frontend/components/ui/`, `tailwind.config.ts`) — кинь сообщение в командный чат, что меняешь, и мержь. Без церемонии.
